...
## ⚡ Quick Guide: Changing Test Limits
If you want to change how many scripts run in the test workflow:
1. Open the **"Find Product Test Files"** node.
2. Find the line: `const testFiles = allFiles.slice(0, 5);`
3. Change `5` to your desired number (e.g., `10`), or remove `.slice(0, 5)` to run **all** scripts.
4. Click **Execute step** and then **Save** the workflow.

---

# 📖 ERP Automation - Detailed Documentation
...
### 7. Recent Stability Updates (Feb 19, 2026)

#### 🚀 Direct Binary Reporting
- **Issue**: The "Read HTML Report" node frequently failed on Windows with "file has changed and cannot be accessed" errors due to filesystem locking during rapid write/read cycles.
- **Fix**: Removed the "Read HTML Report" node. The "Write HTML Report" Code node now generates the binary data directly in memory (`await this.helpers.prepareBinaryData`) and passes it to the Gmail node.
- **Benefit**: 100% reliable email attachments without disk-read race conditions.

#### 📈 Accurate Statistics Calculation
- **Issue**: Negative failed counts or 0 total cases when Playwright summary was missing.
- **Fix**: The "Run Playwright Test" node now manualy sums up `Passed + Failed + Skipped` tests for every run.
- **Benefit**: Guaranteed positive integers and accurate totals regardless of JSON output format.
---
...
| **Scheduler not running** | Ensure the **"Active"** toggle in the top-right corner of the n8n UI is switched **ON**. |
