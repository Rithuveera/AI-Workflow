...
const workflow = {
    "name": "Ultimate AI RCA - Guaranteed Sequential Engine",
    "nodes": [
        {
            "parameters": {},
            "id": "trigger",
        ...
        },
        {
            "parameters": {
                ...
                "emailType": "html",
                "message": "={{ \"<html><body style='font-family: Arial;'><div style='max-width: 800px; margin: auto;'> <div style='text-align:center; padding: 20px; background: #2c3e50; color: white;'><h2>🚀 Execution Dashboard</h2></div> <div style='padding: 20px; border: 1px solid #ddd;'>\" + $json.html + \"</div></div></body></html>\" }}"
            },
            "id": "gmail_summary",
        ...
        },
        {
            "parameters": {
                ...
                "sendBody": true,
                "specifyBody": "json",
                "jsonBody": "={{ JSON.stringify({ text: ($json.status === 'passed' ? '✅' : '❌') + ' *Case:* ' + $json.testTitle + '\\n*File:* ' + $json.filename + '\\n*Status:* ' + $json.status.toUpperCase() + '\\n*Duration:* ' + $json.duration }) }}"
            },
            "id": "chat_case",
        ...
        },
        {
            "parameters": {
                ...
                "emailType": "html",
                "message": "={{ \"<html><body><h2>❌ Failure Alert</h2><p><b>File:</b> \" + $json.filename + \"</p><p><b>Case:</b> \" + $json.testTitle + \"</p><h3>🤖 AI Root Cause Analysis:</h3><div>\" + $json.aiAnalysis.replace(/\\n/g, '<br>') + \"</div><h3>Logs:</h3><pre>\" + $json.error + \"</pre></body></html>\" }}",
                "attachmentsUi": {
                    "attachmentsValues": [
                        {
        ...
        },
        {
            "parameters": {
                ...
                "subject": "={{ '🚨 Failed: ' + $json.testTitle }}",
                "emailType": "html",
                "message": "={{ \"<html><body><h2>❌ Failure Alert (No Img)</h2><p><b>File:</b> \" + $json.filename + \"</p><p><b>Case:</b> \" + $json.testTitle + \"</p><h3>🤖 AI Root Cause Analysis:</h3><div>\" + $json.aiAnalysis.replace(/\\n/g, '<br>') + \"</div><h3>Logs:</h3><pre>\" + $json.error + \"</pre></body></html>\" }}"
            },
            "id": "gmail_no_image",
        ...
        },
        {
            "parameters": {
                ...
                "sendBody": true,
                "specifyBody": "json",
                "jsonBody": "={{ JSON.stringify({ text: '❌ *Failed:* ' + $json.testTitle + '\\n🤖 *RCA:* ' + $json.aiAnalysis }) }}"
            },
            "id": "chat_fail_case",
            "name": "Google Chat Case RCA",
            "type": "n8n-nodes-base.httpRequest",
            "typeVersion": 4,
            "position": [2950, 700]
        }
    ...
    "connections": {
        ...
        "Find Test Files": {
            "main": [[{ "node": "Master Playwright Engine", "type": "main", "index": 0 }]]
        },
        "Master Playwright Engine": {
            "main": [[{ "node": "Aggregate Summary & Failures", "type": "main", "index": 0 }]]
        },
        "Aggregate Summary & Failures": {
            "main": [
                [
                    { "node": "Gmail Final Dashboard", "type": "main", "index": 0 },
                    { "node": "Extract ALL Cases", "type": "main", "index": 0 },
                    { "node": "Extract Failed Cases", "type": "main", "index": 0 }
                ]
            ]
        },
        "Extract ALL Cases": {
            "main": [[{ "node": "Chat Dispatch Loop", "type": "main", "index": 0 }]]
        },
        "Chat Dispatch Loop": {
            "main": [[{ "node": "Chat Throttle Wait", "type": "main", "index": 0 }]]
        },
        "Chat Throttle Wait": {
            "main": [[{ "node": "Google Chat Case Status", "type": "main", "index": 0 }]]
        },
        "Google Chat Case Status": {
            "main": [[{ "node": "Chat Dispatch Loop", "type": "main", "index": 0 }]]
        },
        "Extract Failed Cases": {
            "main": [[{ "node": "RCA & Email Loop", "type": "main", "index": 0 }]]
        },
        "RCA & Email Loop": {
            "main": [
        ...
        "AI RCA (Gemini)": {
            "main": [[{ "node": "Extract RCA text", "type": "main", "index": 0 }]]
        },
        "Extract RCA text": {
        ...
        "Gmail Individual RCA (Img)": {
            "main": [[{ "node": "Google Chat Case RCA", "type": "main", "index": 0 }]]
        },
        "Gmail Individual RCA": {
            "main": [[{ "node": "Google Chat Case RCA", "type": "main", "index": 0 }]]
        },
        "Google Chat Case RCA": {
            "main": [[{ "node": "RCA & Email Loop", "type": "main", "index": 0 }]]
        }
    }
};

fs.writeFileSync('C:/Users/veeramani/.gemini/antigravity/scratch/Automation Workflow/workflows/n8n-ultimate-ai-rca-guaranteed.json', JSON.stringify(workflow, null, 2));
console.log('Saved n8n-ultimate-ai-rca-guaranteed.json');
